...
public class LoadViewInformation : MonoBehaviour
{
    [Header("Settings")]
    [SerializeField] private Text targetText; // Reference to the Text component.
    [SerializeField] private RectTransform rectTransform; // Reference to the RectTransform component.
    [SerializeField] private LoadViewInformation loadViewInformation; // Reference to the LoadViewInformation script.
    [Space(10)]
    ...
    [SerializeField] private LanguageFontListData fontListObject; // Reference to a list of fonts.
    ...
    private void OnDisable()
    {
        ...
        if (targetText != null)
        {
            ...
            targetText.font = fontListObject.fontList[0]; // Set the font to the previously selected font from the font list object.
    ...
    public void CollectInformation()
    {
        ...
        if (targetText != null)
        {
            ...
            if (targetText.alignment == TextAnchor.UpperLeft || targetText.alignment == TextAnchor.MiddleLeft || targetText.alignment == TextAnchor.LowerLeft)
            {
                oldAlignmentValue = 1;
            }
            else if (targetText.alignment == TextAnchor.UpperCenter || targetText.alignment == TextAnchor.MiddleCenter || targetText.alignment == TextAnchor.LowerCenter)
            {
                oldAlignmentValue = 2;
            }
            else if (targetText.alignment == TextAnchor.UpperRight || targetText.alignment == TextAnchor.MiddleRight || targetText.alignment == TextAnchor.LowerRight)
            {
                oldAlignmentValue = 3;
            }
        }
    ...
    private void SetFont()
    {
        if (font != 0)
        {
            if (fontListObject != null && font > 0 && font <= fontListObject.fontList.Count) // Check if fontListObject and valid font index are available.
            { targetText.font = fontListObject.fontList[(int)font - 1]; } // Set the font to the new provided font from the font list.
        }
        else { targetText.font = fontListObject.fontList[0]; } // If new font value is 0 or invalid, revert to the default font from the font list.
    }
    ...
    private void SetTextAlignment()
    {
        ...
        {
            string verticalAlignment = TextAlignmentConverter.GetVerticalAlignmentFromTextAlignment(targetText.alignment); // Get the vertical alignment based on the current text alignment.
            TextAnchor newAlignment = TextAnchor.UpperLeft; // Default the new alignment to UpperLeft.
