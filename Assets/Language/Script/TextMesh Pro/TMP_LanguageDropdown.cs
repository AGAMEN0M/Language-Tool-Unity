...
{
    #if UNITY_EDITOR
    [CustomEditor(typeof(TMP_LanguageDropdown))]
    public class TMP_LanguageDropdownEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            TMP_LanguageDropdown script = (TMP_LanguageDropdown)target; // Get the target LanguageDropdown script.
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("TMP_languageDropdown"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("TMP_itemText"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("TMP_captionText"));
        ...
        public void AddLanguageSaveID(TMP_LanguageDropdown script)
        {
            ...
            foreach (TMP_LanguageOptions id in script.options)
            {
    ...
    [Header("Settings")]
    public TMP_Dropdown TMP_languageDropdown; // Reference to the Dropdown UI component to show language options.
    public TMP_Text TMP_itemText; // Reference to the Text component used for displaying the selected item.
    public TMP_Text TMP_captionText; // Reference to the Text component used for displaying the selected caption.
    public List<TMP_LanguageOptions> options = new() { new TMP_LanguageOptions { text = "Option A", sprite = null, ID = -3 }, new TMP_LanguageOptions { text = "Option B", sprite = null, ID = -4 }, new TMP_LanguageOptions { text = "Option C", sprite = null, ID = -5 } }; // List of LanguageOptions representing language options.
    ...
    private void OnEnable()
    {
        TMP_itemText = TMP_languageDropdown.itemText; // Get the Text component of the Dropdown item.
        TMP_captionText = TMP_languageDropdown.captionText; // Get the Text component of the Dropdown caption.
    ...
    public void LanguageUpdate()
    {
        ...
        foundID = false;
        foreach (TMP_LanguageOptions p in options)
        {
            p.foundID = false;
        }
        ...
        if (File.Exists(savePath))
        {
            ...
            TMP_languageDropdown.value = previousSelectedIndex; // Set the value of the Dropdown to the previously selected index.
            TMP_languageDropdown.RefreshShownValue(); // Refresh the Dropdown UI to show the selected language.
        }
        else
        {
    ...
    {
        previousSelectedIndex = TMP_languageDropdown.value; // Store the previous selected index.
        TMP_languageDropdown.ClearOptions(); // Clear the dropdown options.

        // Add the updated language options to the dropdown.
        foreach (TMP_LanguageOptions languageOption in options)
        {
            ...
            TMP_languageDropdown.options.Add(optionData);
        }
    }
    ...
    private void ProcessLanguageOption(string line)
    {
        ...
        {
            TMP_itemText.font = fontListObject.TMP_fontList[font - 1];
            TMP_captionText.font = fontListObject.TMP_fontList[font - 1];
        }

        if (TMP_languageDropdown.TryGetComponent<RectTransform>(out var rectTransform))
        {
    ...
    private void SetTextAlignment()
    {
        ...
        if (alignment >= 1 && alignment <= 3)
        {
            ...
            string verticalAlignment = TMP_TextAlignmentConverter.GetVerticalAlignmentFromTextAlignment(TMP_itemText.alignment);
            ...
            TMP_itemText.alignment = newAlignment;
            TMP_captionText.alignment = newAlignment;
        }
    }

    public void SavaDropdown()
    {
        RectTransform rectTransform = TMP_languageDropdown.GetComponent<RectTransform>(); // Get the RectTransform for positioning and sizing.
        ...
        TMP_Text text = TMP_languageDropdown.captionText.GetComponent<TMP_Text>();
        ...
        TMP_FontAsset fontTemp = text.font;
...
[System.Serializable]
public class TMP_LanguageOptions
{
