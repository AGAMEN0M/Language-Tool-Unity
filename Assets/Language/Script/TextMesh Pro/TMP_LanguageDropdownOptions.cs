...
[AddComponentMenu("Language/UI/TextMesh Pro/Language Dropdown Options (TMP)")]
public class TMP_LanguageDropdownOptions : MonoBehaviour
{
    [Header("Settings")]
    [SerializeField] private TMP_Dropdown TMP_languageDropdown; // Reference to the TMP_Dropdown UI component to show language options.
    [Space(10)]
    ...
    private void Start()
    {
        ...
        TMP_languageDropdown.ClearOptions(); // Clear any existing options in the TMP_Dropdown.
            ...
            foreach (string buildFilePath in files)
            {
                foreach (string unityFilePath in files)
                {
                    ...
                    if (systemLanguage == unityFileLine2)
                    {
                        ...
                        TMP_LanguageFileSave fileSaveData = new()
                        {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            foreach (string line in lines)
            {
                if (line.StartsWith("Linguagem - ["))
                {
                    ...
                    TMP_languageDropdown.options.Add(new TMP_Dropdown.OptionData(language)); // Add language options to the TMP_Dropdown.
        ...
        if (File.Exists(savePath) && File.Exists(fileSavePath))
        {
            ...
            TMP_LanguageSave saveData = JsonUtility.FromJson<TMP_LanguageSave>(json);
            selectedLanguage = saveData.selectedLanguage;

            json = File.ReadAllText(fileSavePath);
            TMP_LanguageFileSave fileSaveData = JsonUtility.FromJson<TMP_LanguageFileSave>(json);
            selectedFile = fileSaveData.selectedFile;

            for (int i = 0; i < TMP_languageDropdown.options.Count; i++)
            {
                if (TMP_languageDropdown.options[i].text == selectedLanguage)
                {
                    TMP_languageDropdown.value = i; // Set the TMP_Dropdown value to the saved language.
                    TMP_languageDropdown.RefreshShownValue();
                    break;
                }
            }
        }
        else
        {
            ...
            {
                TMP_languageDropdown.value = defaultLanguageIndex; // Set the TMP_Dropdown value to the default language.
                TMP_languageDropdown.RefreshShownValue();
                selectedLanguage = languageName;
                selectedFile = files[defaultLanguageIndex];
                OnLanguageChanged(); // Call the method to update the language based on the selected file.
                OnFileChanged(); // Save the selected file to JSON.
            }
        }
        TMP_languageDropdown.onValueChanged.AddListener(delegate {OnLanguageChanged(); } ); // Add a listener for the TMP_Dropdown value change.
    }

    public void OnLanguageChanged()
    {
        selectedLanguage = TMP_languageDropdown.options[TMP_languageDropdown.value].text; // Get the selected language from TMP_Dropdown.
    ...
    private void LanguageUpdate()
    {
        ...
        TMP_LanguageText[] languageTextTextMeshPro = FindObjectsOfType<TMP_LanguageText>();
        TMP_LanguageDropdown[] tmp_LanguageText = FindObjectsOfType<TMP_LanguageDropdown>();
        ...
        foreach (TMP_LanguageText language in languageTextTextMeshPro)
        {
            language.LanguageUpdate();
        }

        foreach (TMP_LanguageDropdown language in tmp_LanguageText)
        {
            language.LanguageUpdate();
        }
