...
public class TMP_AdjustSizeToDropdown : MonoBehaviour
{
    [Header("Settings")]
    ...
    [SerializeField] private List<TMP_BrokenTexts> TextList; // List of Text components and their line break status.
    ...
    private void Start()
    {
        ...
        TMP_Text[] textComponents = GetComponentsInChildren<TMP_Text>(); // Get all Texts in the hierarchy.

        // Initialize the list of Text components within the UI element.
        foreach (TMP_Text text in textComponents)
        {
            text.overflowMode = TextOverflowModes.Overflow;
            TMP_BrokenTexts newText = new() { itemLabel = text, brokenText = false };
    ...
    private void FixedUpdate()
    {
        ...
        foreach (TMP_BrokenTexts item in TextList)
        {
            if (item.brokenText)
            {
                textIsBroken = true;
