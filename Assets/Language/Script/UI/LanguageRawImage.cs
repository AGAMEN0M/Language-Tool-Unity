...
{
    [Header("Settings")]
    [SerializeField] private RawImage rawImage; // Reference to the RawImage component used to display the language-specific image.
    [Space(5)]
    ...
    private void Start()
    {
        LanguageUpdate(); // Update the Language RawImage.
    }

    private void OnEnable()
    {
        LanguageUpdate(); // Update the Language RawImage when the component is enabled.
    }
    ...
    private IEnumerator LoadAndSetImage()
    {
        ...
        // Load the image file from the specified path and set it as the texture of the RawImage component.
